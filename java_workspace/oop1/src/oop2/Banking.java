...
public class Banking {
	...
	//출금기능은 출금액과 비밀번호를 전달받아서 비밀번호가 일치하는 경우 잔액을 출금액만큼 감소시키고
	//출금액에서 수수료(1300원)를 뺀 금액을 반환한다.
	...
	//조회기능은 소유주, 계좌번호, 현재 잔액을 화면에 출력한다.
	void display() {
	...
	//비밀번호 변경 기능은 이전 비밀번호와 새 비밀번호를 전달받아서 이전 비밀번호가 일치하는 경우에만 비밀번호를 새 비밀번호로 바꾼다.
	...
	//해지기능은 비밀번호를 전달받아서 비밀번호가 일치하는 경우에만 계좌를 해지하고 금리를 적용한 총 해지금액을 반환한다.
	long close(int pwd) {
		long finalMoney = 0L;
		if(password == pwd) {
			finalMoney = (long) (balance + (balance*Banking.RATE_OF_INTEREST*period/12));
