...
//		addWorkAndPersonCourse(c1);		c1은 파라미터 타입이 Student로 지정되어 있어서 전달 불가
//		addWorkAndPersonCourse(c2);		c2는 파라미터 타입이 PostGraduateStudent로 지정되어 있어서 전달 불가
		addWorkAndPersonCourse(c3);
		addWorkAndPersonCourse(c4);
		
		/*
		 * 와일드카드 제네릭 타입제한
		 *		<? extends T>	상위 제한	T와 T의 자손들
		 *		<? super T>		하위 제한	T와 T의 조상들
		 *		<?>				제한 없음	<? extends Object>와 동일
		 *	* 단, 와일드카드를 사용해서 타입제한을 하기 위해서는 해당 클래스가 제네릭타입 클래스여야 한다.
		 */
		
		
		// <? extends Student>는 Student와 Student 의 자손으로 타입을 제한한다.
...
//		
//		// <? super Worker>는 Worker와 Worker의 부모들로 타입을 제한한다.
