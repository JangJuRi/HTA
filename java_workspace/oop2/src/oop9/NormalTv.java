...
public class NormalTv {
	...
	public NormalTv(Monitor monitor) {			// 생성자를 실행할 때 Monitor를 구현한 객체를 전달받겠다.
	...
	public void watch() {
		...
		// Monitor.display()를 실행하면 실제 멤버변수에 조립된 객체가 NormalMonitor라면 	--> 컬러로 동영상 재생
		//															  HDMonitor라면			--> 4K화질로 동영상 재생
		//															  CinemaMonitor라면		--> 100인치 화면에서 동영상 재생
		//															  LCDMonitor라면		--> 터치가 가능한 100인치 화면에서 동영상 재생이 실행된다.
		
		/*
		 * 다형성(polymorphism)
		 * 1. 조상타입의 참조변수로 여러가지 형태의 자손타입 객체를 참조할 수 있다는 의미이다.
