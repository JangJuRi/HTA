...
public class DataService {
	...
	public void printDataStatByMonth() {
		...
		for(Data data : datas) {
				String day = data.getDay();		// ex) "01-01", "12-23"
				String monthString = day.substring(0, 2);
				int month = 0;
				
				if(monthString.startsWith("0")) {
					month = Integer.parseInt(monthString.substring(1));		// ex) "01-01" -> 1
				} else {
					month = Integer.parseInt(monthString);					// ex) "12-23" -> 12
				}
				
				Stat saveStat = stats.get(month);			// Map에서 month를 key로 Stat객체를 조회한다.
				if(saveStat == null) {						// m으로 저장된 Stat객체가 없으면
					...
					saveStat.setDeaths(saveStat.getDeaths() + data.getDeath());
					saveStat.setRecords(saveStat.getRecords() + 1);
				}
			}
	...
	public void printDataStat() {
		// 최다 사고발생 시도, 최다 사고발생 월, 최다 사고발생일, 최다사고발생 요일, 최다사고발생 유형
