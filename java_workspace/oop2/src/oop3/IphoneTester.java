...
public class IphoneTester {
	...
	void testIphone(Iphone p) {
		...
		if(p instanceof Iphone8) {
			Iphone8 i8 = (Iphone8) p;		// (Iphone) p	<--	p가 참조하는 객체에서 Iphone8객체를 찾아서(형변환) 그 객체의 주소값을 반환한다.
			...
			System.out.println("아이폰X의 카메라 기능 테스트 완료");
			
		} else if(p instanceof IphoneX) {
			((IphoneX)p).faceScan();
			System.out.println("아이폰11의 얼굴인식기능 기능 테스트 완료");
		}		
	}
}
