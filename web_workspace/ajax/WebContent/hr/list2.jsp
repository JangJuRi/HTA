...
<body>
	...
	<script type="text/javascript">
		...
		function openPopup(event) {
			// 링크 클릭 시 페이지가 이동되는 기본동작이 발생하지 않도록 방해함.
			...
			xhr.onreadystatechange = function() {
				// 콜백함수가 4번 실행되기는 하지만, 실질적인 작업이 구현된 아래의 소스는 하나의 XHR에 대해서 단 한번만 실행된다.
				if(xhr.readyState == 4 && xhr.status == 200) {
					// 서버가 보낸 응답데이터 조회 <-- XHR의 responseText는 응답데이터를 보관하고 있다.
					var text = xhr.responseText;
					// 응답데이터가 JSON텍스트라면 JSON.parse(json표기법의 텍스트) 메소드를 실행해서 텍스트데이터를 자바스크립트 배열이나 객체로 변환한다.
		...
		function getEmployees() {
            ...
            */
			...
			xhr.onreadystatechange = function() {	// 이벤트 발생 시 실행될 함수 등록
				if(xhr.readyState == 4 && xhr.status == 200) {
					var text = xhr.responseText;
